...
- **服务端（Next.js API Routes + Prisma ORM）**
...
本系统采用 **Next.js 全栈 + PostgreSQL** 架构：
...
- **Prisma ORM**：类型安全的数据库操作，自动生成类型
- **NextAuth.js**：安全的 Session 管理和认证
- **Route Handlers**：RESTful API 端点在 `app/api/*` 目录下
- **本地文件存储**：用户上传的图片存储在 `public/uploads/`
...
- 后台可查看用户评价并（可选）回复
- 已实现 Next.js App Router API（Prisma + Postgres）：`POST /api/reviews`、`GET /api/reviews/user`、`GET /api/reviews/order/:orderId`、`GET /api/reviews/featured`
...
2. 前端调用 Next.js API Route，向 `Order` 表插入一条记录。
3. 管理员在后台查看订单，将状态更新为 `"In Progress"`。
4. 订单完成后，管理员将状态更新为 `"Completed"`。
5. 系统自动执行：
   - 扣减对应球线库存（更新 `StringInventory.stock`）。
   - 为用户增加积分（写入 `PointsLog` 并更新 `User.points`）。
   - 根据售价和成本价计算利润，并写入 `Order.profit`。
...
2. 系统处理：
   - 将 inviter 与 invitee 建立关联（写入 `ReferralLog` 或 `User.referredBy`）。
   - 分别为邀请人和被邀请人增加积分（写入 `PointsLog` 并更新 `User.points`）。

---

## 6. 数据库设计（Database Schema）

采用 **PostgreSQL + Prisma ORM**，以下为核心表结构概述（完整定义见 `prisma/schema.prisma`）。
...
以下为核心业务 API 示例（实现在 `src/app/api/*`）。
...
通过 NextAuth.js + API 中间件限制数据访问范围：
...
- API Routes 使用 `requireAuth()` 中间件验证用户身份
- Admin Routes 额外检查 `session.user.role === 'admin'`
- Prisma 查询自动过滤 `userId` 确保数据隔离

### 8.2 常见安全措施

- 使用 NextAuth.js Session 认证（JWT 模式）。
- 全站 HTTPS Only。
- API Rate Limiting 防止暴力攻击。
- 密码使用 bcrypt 哈希存储。
- 防止重复支付：使用唯一业务 ID。
- 防止恶意扣库存：所有库存扣减逻辑只在服务端 API 中执行，不在前端直接操作。
...
- 为高频查询字段加索引：
  ...
  - `Payment.orderId`
- 使用轮询机制检查订单更新与状态变动。
- Next.js API Routes 支持 Serverless 部署，应对高峰期自动扩容。
- 静态资源使用 Vercel / Netlify CDN 加速。
- 尽量减少 N+1 查询，使用 Prisma `include` 进行关联查询。
...
| 模块                  | 部署位置                           |
| --------------------- | ---------------------------------- |
| Next.js 全栈应用      | Vercel / Railway / Docker         |
| PostgreSQL 数据库     | Docker / Railway / Supabase DB    |
| 图片 / 球线照片       | 本地存储 `/public/uploads/`        |

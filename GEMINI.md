🧠 GEMINI.md — Gemini Coding Agent Development Protocol
...
此文档用于规范项目中所有 Gemini Agent 的开发行为，使其能够：

- 在每次开发前 **自动理解整个项目上下文**  
- 在开发执行过程中 **遵守一致的架构、风格与规范**  
- 在开发结束后 **产出系统级文档并放入 `docs/` 文件夹**  
- 确保未来所有 Gemini Agent 都能基于历史文档持续开发，而不会偏离项目方向  

此文档是项目开发的 **唯一行为准则（Single Source of Truth for Gemini Agents）**。

---

## 📌 2. 开发前必须执行的步骤（MANDATORY）

每次开始写代码前，Gemini Agent 必须执行以下动作：
...
Gemini Agent 必须阅读这些文件并建立上下文。

### ✔ STEP 2：理解或更新以下全局信息

Gemini Agent 需要对以下内容有清晰理解（或在变更时同步更新文档）：
...
> Gemini Agent **不得在不了解上下文的情况下开始编码**。

### ✔ STEP 3：生成 “Development Plan”（开发任务计划）

在编写代码前，Gemini Agent 必须输出：
...
docs/
  ...
  api_spec.md
  change_log_*.md
GEMINI.md             # 本文件（Gemini Agent 行为规范）
```

---

## 📌 6. Gemini Agent 输出格式规范（Response Format）

无论需求多小，Gemini Agent 输出必须符合以下结构：
...
## 📌 8. Gemini Agent 行为总结（Core Behavior Summary）

Gemini Agent 在本项目中扮演：
...
- 避免重复造轮子，复用既有组件与模块。  
- 严格参照文档执行，保持结构与风格一致性。  
- 每次开发都为“未来的 Gemini Agent”留下清晰的上下文。  

---

## 📌 9. 视觉设计系统（Visual Design System）

项目采用 **呼吸感设计（Breathing Design）**，Gemini Agent 在开发 UI 时必须遵循：

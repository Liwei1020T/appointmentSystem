...
export default function ProfilePage() {
  ...
  const loadData = async () => {
    ...
    try {
      const [profileResult, statsResult, codeResult, membershipResult, badgesResult] = await Promise.all([
        getUserProfile(),
        getUserStats(),
        generateReferralCode(),
        getMembershipDetails(),
  ...
  return (
    <div className="min-h-screen bg-ink pb-24">
      ...
      <div className="max-w-2xl mx-auto px-4 -mt-6 space-y-4 relative z-20">
        ...
        {/* ========== 成就徽章展示 ========== */}
        {badgesData && badgesData.allBadges.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-border-subtle overflow-hidden">
            <div className="px-4 py-3 border-b border-border-subtle flex items-center justify-between">
              ...
              <span className="text-xs text-text-tertiary">
                {badgesData.totalEarned}/{badgesData.totalAvailable}
              </span>
            </div>
            <div className="p-4">
                  ...
                  <div
                    ...
                    className={`flex flex-col items-center p-3 rounded-xl transition-all ${
                      badge.earned
                        ...
                        : 'bg-ink border border-transparent opacity-50'
                    }`}
                  >
                    <span className={`text-2xl mb-1 ${badge.earned ? '' : 'grayscale'}`}>
                      {badge.icon}
                    </span>
